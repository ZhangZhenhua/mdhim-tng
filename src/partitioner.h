...
rangesrv_list *get_range_servers(struct index_t *index, void *key, int key_len);
rangesrv_info *get_range_server_by_slice(struct index_t *index, int slice);
void build_alphabet();
int verify_key(uint64_t max_recs_per_slice, int key_type, void *key, int key_len);
...
rangesrv_list *get_range_servers_from_stats(struct index_t *index,
