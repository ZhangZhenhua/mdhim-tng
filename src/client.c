...
struct mdhim_brm_t *client_open(struct mdhim_openm_t **openmsg_list,
	...
	for (i = 0; i < num_range_svrs; i++) {
		...
		srvs[num_srvs] = openmsg_list[i]->basem.server_rank;
		num_srvs++;
	}
	...
	return_code = send_all_rangesrv_work(&mdhim_gdata, (void **)openmsg_list, num_range_svrs);
	...
	if (return_code != MDHIM_SUCCESS) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: %d from server while sending "
		     "open request",  mdhim_gdata.mdhim_rank, return_code);
		return NULL;
	}
	...
	if (return_code != MDHIM_SUCCESS) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: %d from server while receiving "
		     "open requests",  mdhim_gdata.mdhim_rank, return_code);
	}
	...
	for (i = 0; i < num_range_svrs; i++) {
		...
		if (!rm) {
		  mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - "
		       "Error: did not receive a response message in client_open",
...
struct mdhim_brm_t *client_close(struct mdhim_closem_t **closemsg_list,
	...
	for (i = 0; i < num_range_svrs; i++) {
		...
		srvs[num_srvs] = closemsg_list[i]->basem.server_rank;
		num_srvs++;
	}
	...
	return_code = send_all_rangesrv_work(&mdhim_gdata, (void **)closemsg_list, num_range_svrs);
	// If the send did not succeed then log the error code and return MDHIM_ERROR
	if (return_code != MDHIM_SUCCESS) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: %d from server while sending "
		     "close request",  mdhim_gdata.mdhim_rank, return_code);
		return NULL;
	}
	...
	for (i = 0; i < num_range_svrs; i++) {
		...
		if (!rm) {
		  mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - "
		       "Error: did not receive a response message in client_close",
		       mdhim_gdata.mdhim_rank);
		  //Skip this as the message doesn't exist
		  continue;
		}
